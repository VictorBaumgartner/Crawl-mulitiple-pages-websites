...
def clean_markdown(md_text):
    """
    Cleans Markdown content by removing links, images, footnotes, and formatting.
    """
    md_text = re.sub(r'\[([^\]]+)\]\((http[s]?://[^\)]+)\)', r'\1', md_text)
    md_text = re.sub(r'http[s]?://\S+', '', md_text)
    md_text = re.sub(r'!\[([^\]]*)\]\((http[s]?://[^\)]+)\)', '', md_text)
...
async def crawl_website(start_url, output_dir, max_pages=100, max_concurrency=8):
    """
    Crawl a website and save each page as a cleaned Markdown file.
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    def sanitize_filename(url):
        ...
        filename = re.sub(r'[<>:"/\\|?*]', '_', filename)
        filename = filename.strip('_') or "index"
        return f"{filename[:250]}.md"

    async def crawl_page(current_url):
        if current_url in visited_urls or len(visited_urls) >= max_pages:
            logger.info(f"Skipping (visited or max pages {len(visited_urls)}/{max_pages}): {current_url}")
            return
        visited_urls.add(current_url)
        ...
        async with semaphore:
            try:
                ...
                async with AsyncWebCrawler(verbose=True, headers=headers) as crawler:
                    result = await crawler.arun(url=current_url, config=config)
                if result.success:
                    raw_markdown = result.markdown.raw_markdown
                    logger.info(f"Raw Markdown length for {current_url}: {len(raw_markdown)}")
                    cleaned_markdown = clean_markdown(raw_markdown)
                    logger.info(f"Cleaned Markdown length for {current_url}: {len(cleaned_markdown)}")
                    filename = sanitize_filename(current_url)
                    output_path = os.path.join(output_dir, filename)
                    try:
                        with open(output_path, "w", encoding="utf-8") as f:
                            f.write(f"# {current_url}\n\n**Crawled At:** {datetime.utcnow().isoformat()}\n\n{cleaned_markdown}\n")
                        logger.info(f"Saved Markdown to: {output_path}")
                    except IOError as e:
                        logger.error(f"Failed to save file {output_path}: {e}")
                        errors.append({"url": current_url, "error": f"IOError: {str(e)}"})
                    internal_links = result.links.get("internal", [])
                    ...
                    for link in internal_links:
                        ...
                        if parsed_url.scheme not in ['http', 'https']:
                            continue
                        if parsed_url.netloc == start_netloc and absolute_url not in visited_urls and absolute_url not in queued_urls:
                            logger.debug(f"Queueing new link: {absolute_url}")
                            queued_urls.add(absolute_url)
                            await crawl_queue.put(absolute_url)
                else:
                    logger.error(f"Failed to crawl {current_url}: {result.error_message}")
                    errors.append({"url": current_url, "error": result.error_message})
            except Exception as e:
                logger.error(f"Error processing {current_url}: {e}", exc_info=True)
    ...
    async def worker():
        while len(visited_urls) < max_pages:
            try:
                ...
                logger.info(f"Worker {asyncio.current_task().get_name()} cancelled.")
                break
            except Exception as e:
    ...
    tasks = [asyncio.create_task(worker(), name=f"worker-{i}") for i in range(max_concurrency)]
    logger.info("Waiting for crawl queue to finish...")
    await crawl_queue.join()
    logger.info("Crawl queue is empty.")
    ...
    try:
        with open(metadata_path, "w", encoding="utf-8") as f:
            json.dump({
                ...
                "queued_urls_at_end": list(queued_urls),
                "errors": errors,
...
async def scrape_website(request: ScrapingRequest):
    """
    FastAPI endpoint to scrape a website and save Markdown files.
    """
    logger.info(f"Received scraping request for URL: {request.url}, folder: {request.folder_name}")
    try:
        target_url = request.url
        ...
        logger.info(f"Scraping completed for {target_url}")
        return {"status": "success", "message": f"Scraping completed for {target_url}. Output saved to {output_dir}"}
    except HTTPException as http_exc:
        raise http_exc
    except Exception as e:
        logger.error(f"Unhandled error during scraping: {e}", exc_info=True)
