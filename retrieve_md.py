...
app = FastAPI(
    ...
    description="An API to crawl websites, including dynamic pages, with Playwright and stealth to bypass bot detection. Optimized with Numba for faster processing.",
    version="1.3.0"
)
...
class CrawlRequest(BaseModel):
    ...
    """
    url: HttpUrl = Field(..., example="https://books.toscrape.com/")  # Starting URL
    max_pages: int = Field(3000, ge=1, example=100)  # Max pages to crawl
    concurrency_limit: int = Field(4, ge=1, le=20, example=4)  # Parallel requests, lowered for Playwright
...
class CustomWebCrawler:
    ...
    async def initialize_browser(self):
        """Initialize Playwright and a single browser instance."""
        try:
            self.playwright = await async_playwright().start()
            self.browser = await self.playwright.chromium.launch(headless=True)
            logger.info("Initialized Playwright browser")
        except Exception as e:
            error_msg = f"Failed to initialize Playwright browser: {e}"
    ...
    async def _fetch_page_content(self, url: str) -> Optional[str]:
        """Fetch page content using Playwright with stealth to bypass bot detection."""
        if not self.browser:
        ...
        try:
            async with self.semaphore:
                logger.info(f"Fetching with Playwright (stealth): {url}")
                ...
                try:
                    ...
                    # Check for bot detection (e.g., 403, CAPTCHA)
                    if response and response.status in [403, 429]:
                        error_msg = f"Possible bot detection at {url}: HTTP {response.status}"
                        logger.warning(error_msg)
                        self.errors.append(error_msg)
                        return None
                    # Check for CAPTCHA or block page content
                    content = await page.content()
                    if any(keyword in content.lower() for keyword in ["captcha", "access denied", "blocked"]):
                        error_msg = f"Bot detection likely at {url}: CAPTCHA or block page detected"
                        logger.warning(error_msg)
                        self.errors.append(error_msg)
                        return None
...
async def crawl_all_markdowns(request: CrawlRequest):
    ...
    Uses Playwright with stealth to bypass bot detection and Numba for optimized processing.
    """
