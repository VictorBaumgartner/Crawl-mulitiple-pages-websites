...
async def save_scraped_data(data: Dict[str, Any], output_path: str, filename_base: str) -> Dict[str, str]:
    ...
    metadata_to_save = {"url": data.get("metadata", {}).get("url", ""), "scrape_time": data.get("metadata", {}).get("scrape_time", ""), "title": data.get("metadata", {}).get("title"), "description": data.get("metadata", {}).get("description"), "keywords": data.get("metadata", {}).get("keywords"), "author": data.get("metadata", {}).get("author"), "og_tags": data.get("metadata", {}).get("og_tags", {}), "twitter_tags": data.get("metadata", {}).get("twitter_tags", {}), "links": data.get("links", []), "images": data.get("images", [])}
...
async def scrape_multiple_endpoint(request: MultiScrapeRequest):
    ...
    # No need to save the report.json file
    ...
    return {
        ...
        "details": processed_results # Include details in the response instead of a separate file
    }
